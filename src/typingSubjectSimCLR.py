...
Positive pairs: two random 75% sub-samples of the same subject's typing sessions.
    ...
    def permute_histogram_segments(self, data):
        ...
        def permute_section(section_data):
            ...
            def permute_segments():
                ...
                return tf.concat([permuted_data, reshaped_data_2], axis=1)
...
temperature = 0.01
learning_rate = 1e-3
sample_frac = 0.75  # Fraction of subject sessions to sample per view
queue_size = 64    # Memory bank size
moco_momentum = 0.99
USE_AUGMENTATION = True    # Set to False to disable stochastic augmentations (normalize_histogram always runs)
DEBUG = False
DEBUG_SUBJECTS = 20
...
def create_subject_view(sessions: np.ndarray, k2: int, frac: float) -> tuple:
    ...
    sampled = sessions[idx]  # (n_sample, F)
...
class SubjectContrastiveModel(keras.Model):
    ...
    def __init__(self, m: int, k2: int, f: int, temp: float, queue_size: int = 64, moco_momentum: float = 0.99):
        ...
        # Augmenter: always normalize_histogram; stochastic steps gated by USE_AUGMENTATION
        _taug = TypingAugmentation()
        if USE_AUGMENTATION:
            self.augmenter = _taug.get_contrastive_augmenter()
        else:
            # Normalization only
            self.augmenter = keras.Sequential([layers.Lambda(_taug.normalize_histogram)])
...
if __name__ == "__main__":
    ...
    plt.title("t-SNE — Subject Typing SimCLR Embeddings (attention-aggregated)")
